...
考虑怎么解这个东西。只需要不停地拆开就可以得到$T\left(n\right)=3^{\log_{2}{n} }\times
T_0$，其中$T_0$是一个与n无关的常数，所以我们可以认为$T\left(n\right)=3^{\log_{2}{n} }=n^{\log_2{3}
}$，等号的由来可以两边取对数
...
这里讨论形如$T\left(n\right)=f\left(n\right)+\sum\limits_{i=1}^{k}{a_i
T\left(n-i\right)}$的方程的解的结构，当然括号内部还可以更复杂，这里只是一个简单的情形
...
对于有重根的情况，解可以写成$\sum\limits_{i=1}^{k-L}\left({a_i\cdot\sum\limits_{j=1}^{L_i}{\left({n^{j-1}\cdot
x_i}\right)} }\right)$
...
若$x_1$是多项式的二重根，则它是$F'\left(x\right)$的单根，即$F'\left(x\right)=nx^{n-1}-\sum\limits_{i=1}^k{a_i\cdot
i\cdot x^{i-1} }$

两边同事乘上x得到$xF'\left(x\right)=nx^n-\sum\limits_{i=1}^k{a_i\cdot i\cdot
x^i}$，带入$x=x_1$恰好等于0
...
这样的东西就是生成函数(Generating
Function)，也被称为母函数/形式幂级数。在这里我们不关心它是否收敛，而只在乎它的系数，x只是一个占位符号
...
观察第l项的系数，这里是k个等比数列相乘，这k个项的未知数上指数之和为l，p的指数之和也为l，这里用隔板法就可以做了，系数就是$\binom{l+k-1}{k-1}\cdot
p^l$
...
我们称函数$f(x)$的增长速度在渐进意义下不超过$g(x)$，代表$\exists N,M>0$，当$n>N$时，恒有$f(x)\le M\cdot
g(x)$成立，即$\lim\limits_{n\rightarrow \infty}{\frac{f(x)}{g(x)} }< M$，其中$M$
是个常数。
...
还有一个很有意思的结论：$\sum\limits_{i=1}^{k}{f_i(n)}\le\sum\limits_{i=1}^{k}{f_{max}(n)}=k\cdot
f_{max}(n)=\Theta(f_{max}(n))$，这里$k$是和n无关的常数。这个结论写出来就证明完了
...
1.
若$f(n)=\Theta\left(n^c\right)$，且$c<\log_ba$，那么$T\left(n\right)=\Theta\left(n^{\log_ba}\right)$

2. 若$f(n)=\Theta\left(n^c\right)$，且$c>\log_ba$，且$f(n)$满足正则条件(regular
condition)，那么$T\left(n\right)=\Theta\left(n^c\right)$

3. 若$f(n)=\Theta\left(n^{\log_ba}\log^k n\right)$，且$k>
-1$，则$T\left(n\right)=\Theta\left(n^{\log_ba}\log^{k+1}n\right)$


正则条件指的是对于函数$f(n)$，$\exists d< 1$，当n充分大时，满足$a\cdot f\left(\frac{n}{b}\right)<
d\cdot f\left(n\right)$
